...
function [] = main2(N,S,CP,L,index,~,M)
    ...
    %method3 R_HY
    if ismember(index,p_indices)
        R_HYp3 = gen_RHY(h_var,N,p_indices2,p_indices2);
    elseif index > (Np-1)*S
        R_HYp3 = gen_RHY(h_var,N,p_indices2,p_indices2);
        
    else
        R_HYp3 = gen_RHY(h_var,N,p_indices2(1),p_indices2(1));
        R_HYp4 = gen_RHY(h_var,N,p_indices2(2),p_indices2(2));
    ...
    for i = 1: length(snr)
        ...
        Hi_hat = transpose(R_HYp *inv(R_YpYp) * transpose(rx_pilots));
        ...
        if (numel(p_indices2) == 2)
            ...
            Hi_hat2 = transpose(R_HYp2 *inv(R_YpYp2) * transpose(rx_pilots2));
        else
            rx_pilots2 = rx_dft(:,p_indices2);
...
%             size(rx_pilots2)
        ...
        if (numel(p_indices2) == 2)
            ...
            Hi_hat_first = transpose(R_HYp3 * inv(R_YpYp3) * transpose(rx_pilots_first));
    
            rx_pilots_second = rx_dft(:,p_indices2(2));
            R_YpYp4 = R_YpYp2(2,2);
            Hi_hat_second = transpose(R_HYp4  *inv(R_YpYp4) * transpose(rx_pilots_second));
            
            m = (Hi_hat_second - Hi_hat_first)/(p_indices2(2) - p_indices2(1));
            Hi_hat3 = m*(index - p_indices2(1)) + Hi_hat_first;
        else
            rx_pilots_first = rx_dft(:,p_indices2);
            R_YpYp3 = R_YpYp2;
            size(rx_pilots_first)
            size(R_YpYp3)
            size(R_HYp3)
            Hi_hat3 = transpose(R_HYp3 * inv(R_YpYp3) * transpose(rx_pilots_first));
        end
